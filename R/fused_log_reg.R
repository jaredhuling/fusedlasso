...
fusedLassoLogistic <- function(x, y, lambda, class.weights = NULL, opts=NULL) {
  ...
  if (opts$mFlag == 0 && opts$lFlag == 0) {
    ...
    for (iterStep in 1:opts$maxIter) {
      ...
      bb <- max(aa, 0)
      fun.s <- as.double( crossprod(weight, log(exp(-bb) + exp(aa - bb))) ) + 
      ...
      while (TRUE) {
        ...
        fun.beta <- as.double( crossprod(weight, log(exp(-bb) + exp(aa - bb))) ) + 
  ...
  list(beta = drop(beta), funVal = funVal, ValueL = ValueL)
}